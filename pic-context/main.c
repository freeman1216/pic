...
void *blinky_slot_context;

void systick_isr(){
    blinky_context.callable_tick(&api,blinky_slot_context);
}

int __attribute__((noinline,noreturn)) main(){
    __main_clock_setup();
    __periph_setup();
    BAD_ARENA_ALOCATE_STATIC(arena,1024);
    for(uint32_t i = 0; i < slot_len; i++){
        module[i] = slot[i];
    }
    
    blinky_context.header = (slot_header_t *)module; 
    blinky_slot_context = bad_arena_alloc(arena,blinky_context.header->context_size)
    ...
    blinky_context.callable_init(&api,blinky_slot_context);
