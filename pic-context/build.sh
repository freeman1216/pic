...
slotopts="-mcpu=cortex-m4 -mfloat-abi=soft -Tslot.ld -ggdb -Wall -Wextra -fjump-tables -nolibc --specs=nosys.specs -nostartfiles -mthumb"
...
while [ $# -gt 0 ]; do
	case "$1" in
		slot)
			...
			sed -i "s/unsigned char slot\[\]/const unsigned char rom\[\] __attribute__((used,section(\".slot\")))/g" generated/slot.h
			;;
		firm)
			build_firm="true"
			;;
